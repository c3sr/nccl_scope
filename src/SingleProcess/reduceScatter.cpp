...
auto NCCL_ops_reduceScatter = [](benchmark::State &state) {
  ...
  for (int i = 0; i < nDev; ++i) {
    ...
    OR_SKIP(cudaMalloc(sendbuff + i, bytes * sizeof(float) * nDev), NAME " failed to perform cudaMalloc");
    OR_SKIP(cudaMalloc(recvbuff + i, bytes * sizeof(float)), NAME " failed to perform cudaMalloc");
    OR_SKIP(cudaMemset(sendbuff[i], 1, bytes * sizeof(float) * nDev), NAME " failed to perform cudaMemset");
    ...
    OR_SKIP(cudaEventCreate(stops+i), NAME " faield to create events");
  }
  ...
  //initializing NCCL
  NCCL_SKIP(ncclCommInitAll(comms, nDev, devs), NAME " failed to initailize comm");
  for(auto _ : state){
  ...
  for (int i = 0; i < nDev; ++i){
    OR_SKIP(cudaEventRecord(starts[i], s[i]), NAME " failed to record start events");
    NCCL_SKIP(ncclReduceScatter(sendbuff[i], recvbuff[i], bytes, ncclFloat, ncclSum,
        comms[i], s[i]), NAME " failed to perform reduceScatter");
    OR_SKIP(cudaEventRecord(stops[i], s[i]), NAME " failed to record stop events");
  }
...
//  for (auto cuda_id : unique_cuda_device_ids()) {
   // for (auto numa_id : unique_numa_ids()) {
      ...
      benchmark::RegisterBenchmark(name.c_str(), NCCL_ops_reduceScatter)->SMALL_ARGS()->UseManualTime();
