...
#define NAME "NCCL/function/BROADCAST"
//time problems only 
...
static void NCCL_function_BROADCAST(benchmark::State &state) {
  ncclComm_t comms[4];
...
for(auto _ : state){
  ...
  for (int i = 0; i < nDev; ++i){
    ...
    NCCLCHECK(ncclBroadcast(sendbuff[i], recvbuff[i], size, ncclFloat,0,
        comms[i], s[i]));
...
BENCHMARK(NCCL_function_BROADCAST)->Apply(ArgsCountGpuGpuGpuGpu);
