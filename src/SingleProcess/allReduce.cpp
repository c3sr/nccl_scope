...
auto NCCL_ops_allReduce = [](benchmark::State &state) {
  ...
  OR_SKIP(cudaEventRecord(start, NULL), NAME " failed to start event");
  ...
  for (int i = 0; i < nDev; ++i){
    ...
    NCCL_SKIP(ncclAllReduce(sendbuff[i], recvbuff[i], bytes, ncclFloat, ncclSum ,
...
//  for (auto cuda_id : unique_cuda_device_ids()) {
   // for (auto numa_id : unique_numa_ids()) {
      ...
      benchmark::RegisterBenchmark(name.c_str(), NCCL_ops_allReduce)->SMALL_ARGS()->UseManualTime();
...
//BENCHMARK(NCCL_ops_allReduce)->Apply(ArgsCountGpuGpuGpuGpu)->UseManualTime();
