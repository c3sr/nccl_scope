...
static void NCCL_function_ALLGATHER(benchmark::State &state) {
  ...
  //right around here need to set rank
  // need mpi to broadcast id to everyone
  ncclUniqueId id;

  ncclGetUniqueId(&id);
  ncclCommInitRank(comms, 4 , id ,4 );

  //this is for one stream don't do this
...
BENCHMARK(NCCL_function_ALLGATHER)->Apply(ArgsCountGpuGpuGpuGpu)->UseManualTime();
